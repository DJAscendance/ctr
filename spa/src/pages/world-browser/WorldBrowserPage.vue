  ...
  methods: {
    ...
    async renderPresenceAdded(presence: Presence): Promise<void> {
      ...
      loadInlineAsync(browser, avURL, (nodes) => {
        ...
        /*
         * The same node also wears the historical blaxxun label, because a hit
         * path entry is what ne_game.wrl's fire() reads: it looks for an entry
         * answering to 'Avatar' and sends `team + entry.nickname` on the wire.
         * The label is the PRESENCE KEY, never the username - two tabs of one
         * member share a username, and a shot has to name the tab it hit.
         * blaxxunAvatarNameFor() is the other half: it makes the target's own
         * Browser.myAvatarName the same key, so the Script's comparison matches
         * exactly one citizen.
         */
    ...
    /*
     * X_ITE's own supersession message.
     *
     * A world load that is still running when the next one starts is cancelled:
     * loadURL chains into replaceWorld, the newer load installs its own, and
     * when the first file finally arrives the first load sees that it is no
     * longer the current one and rejects with exactly this message. It is a
     * cancellation signal, not a load failure, and it is the only message that
    ...
    async startX3D(generation: number): Promise<any> {
      ...
      /*
       * The world about to be replaced does not get to keep the browser. Its
