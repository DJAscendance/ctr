 ...
 * any guard runs. Writing the store from the guard therefore let a place the citizen never
 * reached land on top of the place they are standing in. Staging it here and committing it
 * from `afterEach` means only the navigation that actually landed can write the store. See
 * `helpers/navigation-place.helper.ts` for why the route object is the navigation's
 * identity.
 */
