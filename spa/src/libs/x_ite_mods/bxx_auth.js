...
(function () {
    ...
    function navigationValue(browser, name) {
        ...
        /* Configurable, because bxx_identity.js layers the Beta identity provider
         * over these two later in the patch order. A non-configurable accessor
         * made that redefinition throw and took the whole identity patch down. */
        ...
        // Navigation
        /*
         * Blaxxun modes seen in the archived worlds are WALK, PAN and NONE.
         ...
         * approximation and is recorded as such.
         *
         * X_ITE 4 changed the viewer by assigning `viewer_`; X_ITE 16 does not
         * expose that lever, so the mode is applied to the bound
         * NavigationInfo's `type` through the same accessor every other
         * navigation method here uses. The assignment to `viewer_` is kept so a
         * world that reads the mode back sees what it asked for even when the
         * NavigationInfo write is refused.
         */
        var BXX_NAVIGATION_MODES = {
            WALK: 'WALK',
            PAN: 'FLY',
            NONE: 'NONE',
            FLY: 'FLY',
        ...
        b.setNavigationMode = function (mode) {
            var requested = String(mode).toUpperCase()
            // Added due to Jail calling this constantly
            if (this.bxxNavigationMode_ === requested) { return }
            this.bxxNavigationMode_ = requested
            this.viewer_ = mode
            var mapped = BXX_NAVIGATION_MODES[requested]
            if (!mapped) { return }
            try {
                var type = navigationField(this, 'type')
                if (typeof type.setValue === 'function') {
                    type.setValue([mapped])
                } else {
                    navigationInfo(this).type = [mapped]
                }
            } catch (err) {
                console.warn('setNavigationMode: could not apply ' + requested, err)
            }
        }
