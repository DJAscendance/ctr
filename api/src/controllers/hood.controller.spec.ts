...
import type { HoodController as HoodControllerType } from './hood.controller';
...
// eslint-disable-next-line @typescript-eslint/no-var-requires
const { HoodController } = require('./hood.controller') as typeof import('./hood.controller');
...
describe('HoodController - map background endpoints', () => {
  ...
  let controller: HoodControllerType;
  ...
  describe('getMapBackgroundOptions', () => {
    it('returns 200 with the resolved options for a valid hood', async () => {
      const request: MockRequest = { params: { id: '60' } };
      const response = mockResponse();
      const payload: MapBackgroundOptionsResult = {
        selectedIndex: null,
        effectiveIndex: 0,
        effectiveUrl: '/assets/img/map_themes/cyberhood/hood/Pimg2D000.gif',
        options: [{ index: 0, url: '/assets/img/map_themes/cyberhood/hood/Pimg2D000.gif' }],
      };
      hoodService.getMapBackgroundOptions.mockResolvedValue(payload);
     ...
     * real hood instead of rejecting the request.
     */
    it.each([
      ['60x', 'a numeric prefix'],
      ['1.5', 'a fraction'],
      ['1e2', 'exponent notation'],
      ['0', 'zero, which no hood uses'],
      ['-60', 'a negative id'],
      ['', 'an empty id'],
    ...
    it('returns 500 when the service fails to read the asset directory', async () => {
      ...
      failure.code = 'EACCES';
      hoodService.getMapBackgroundOptions.mockRejectedValue(failure);
    ...
    it('does not leak the failing filesystem path in the error response', async () => {
      const request: MockRequest = { params: { id: '60' } };
      const response = mockResponse();
      hoodService.getMapBackgroundOptions
    ...
    it('does not require authentication to read options', async () => {
      const request: MockRequest = { params: { id: '60' } };
      const response = mockResponse();
      hoodService.getMapBackgroundOptions.mockResolvedValue({
  ...
  describe('putMapBackgroundSelection', () => {
    it('returns 401 when no token is present', async () => {
      const request: MockRequest = { params: { id: '60' }, headers: {}, body: { index: 1 } };
      const response = mockResponse();
     ...
     * hood, so a truncated id can never be authorized or written.
     */
    it.each([
      ['60x', 'a numeric prefix'],
      ['1.5', 'a fraction'],
    ...
    it('returns 400 when the service reports the index is invalid for this pool', async () => {
      ...
      memberService.decodeMemberToken.mockReturnValue(session);
      hoodService.canAdmin.mockResolvedValue(true);
      hoodService.updateMapBackgroundSelection
    ...
    it('returns 500 when the write path hits a filesystem fault', async () => {
      ...
      const failure: NodeJS.ErrnoException = new Error('EACCES');
      failure.code = 'EACCES';
      hoodService.updateMapBackgroundSelection.mockRejectedValue(failure);
    ...
    it('returns 404 when the service cannot resolve the hood/colony', async () => {
      const request: MockRequest = {
        params: { id: '60' },
        headers: { apitoken: 'token' },
        body: { index: 1 },
    ...
    it('returns 200 with the normalized selection on success', async () => {
      ...
      expect(hoodService.updateMapBackgroundSelection).toHaveBeenCalledWith(60, 2);
      expect(response.status).toHaveBeenCalledWith(200);
    ...
    it('validates authorization against the URL hood id, not any client-supplied id', async () => {
      const request: MockRequest = {
        params: { id: '61' },
        headers: { apitoken: 'token' },
        body: { index: 1, id: 999 },
      ...
      expect(hoodService.canAdmin).toHaveBeenCalledWith(61, 42);
      expect(response.status).toHaveBeenCalledWith(403);
    });
  });
});
