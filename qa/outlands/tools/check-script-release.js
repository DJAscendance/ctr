...
/*
 * CTR_BETA_OUTLANDS_SCRIPT_RELEASE
 *
 ...
 * with a shutdown(); no other asset CTR serves has any.
 *
 ...
 *     node qa/outlands/tools/check-script-release.js [outDir]
 */
...
const OUT_DIR = process.argv[2]
  || path.join(REPO, '..', '..', '..', 'artifacts', 'outlands-script-release');
 ...
 * visible. Identity and not name: Outlands has fourteen Scripts sharing two
 * names, and a DEF/USE pair is one node reachable by two paths.
 *
...
(async () => {
  ...
  /* ---- first visit ---- */
  await O.enterOutlandsThroughEntrance(page, 'redm', 120000);
  await O.waitForBattleTeam(page, 60000);
  ...
  const insideState = await O.state(page);
  check('the battle Script is running on the first visit',
    insideState.team > 0 && insideState.browserEventRoutes > 0 && insideState.eventMask > 0,
    { team: insideState.team, routes: insideState.browserEventRoutes, mask: insideState.eventMask });
  ...
  await O.enterOutlandsThroughEntrance(page, 'bluef', 120000);
  await O.waitForBattleTeam(page, 60000);
  await page.waitForTimeout(1500);

  const again = await snapshot(page);
  const againListeners = await unloadListeners(cdp);
  const againState = await O.state(page);
  check('the new battle Script initializes and runs on re-entry',
    againState.team > 0 && againState.ammo.beamer > 0,
    { team: againState.team, ammo: againState.ammo });
  ...
  check('the new world gets its own browser event route and mask',
    againState.browserEventRoutes > 0 && againState.eventMask > 0,
    { routes: againState.browserEventRoutes, mask: againState.eventMask });
